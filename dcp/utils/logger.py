...
_logging_lvl_from_env = os.environ.get("DCP_DEBUG", "INFO")
...
_print_debug_dir = False
if _default_logging_level == logging.DEBUG:
    debug_dir = os.environ.get("DCP_LOGGING_DEBUG_DIR")
    if debug_dir is None:
        debug_dir = os.path.join(os.getcwd(), "debug")
        # _print_debug_dir = True
...
class DCPFormatter(logging.Formatter):
    white = "\x1b[38;20m"
    grey = "\x1b[38;5;8m"
    yellow = "\x1b[33;20m"
    red = "\x1b[31;20m"
...
def create_logger(
    ...
    if log_file is not None:
        ...
        handler = logging.FileHandler(filename=full_log_path, mode="w")
        ...
        logger_.log_file = full_log_path
    else:
        ch = logging.StreamHandler(stream=sys.stdout)
        ch.setLevel(level)
        ch.setFormatter(colored_formatter)
        handlers.append(ch)
        logger_.log_file = None
...
logger = create_logger(name="DCP", level=_default_logging_level)
if _print_debug_dir:
    logger.info(
        "DCP_LOGGING_DEBUG_DIR not set. "
        f"Debug logs will be saved to {_debug_log_dir}"
    )
