...
/usr/local/cuda/bin/nsys profile -w true -t cuda,nvtx,osrt,cudnn,cublas -s cpu -o nsight_report -f true --cuda-event-trace false --capture-range=cudaProfilerApi -x true \
python -m torch.distributed.launch \
    ...
    --use-env \
    benchmark/microbenchmark/benchmark_attention.py --profile -nd ${nproc_per_node} "$@"
